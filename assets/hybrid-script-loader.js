// Système Hybride de Chargement Intelligent + Debug/Performance
(function () {
  console.log("[INIT] Initializing Hybrid Intelligent Script Loader...");

  // ===== NOUVELLE SECTION: A/B + Performance Controls =====
  ...
  (function(){
    const m = location.search.match(/[?&]loader=(script|fetch|cdn|fallback)\b/i);
  ...
  const scripts = [
    ...
    },
    {
      ...
      fallback: () => {
        ...
        // Convertir liens GLightbox en liens normaux
        document
  ...
  // ===== NOUVEAU: Helpers de Performance =====
  ...
  window.hybridReport = function() {
    const entries = performance.getEntriesByType('measure')
      ...
      .map(e => ({ script: e.name.replace('load:', ''), ms: Math.round(e.duration) }));

    console.table(entries);
    const total = entries.reduce((a,b) => a + b.ms, 0);
    console.log('Total load (ms):', total, '| mode force =', window.HYBRID_LOADER.force || 'auto');
  };

  // ===== MISE À JOUR: loadScriptHybrid avec Performance + Force Mode =====
  async function loadScriptHybrid(script) {
    const F = window.HYBRID_LOADER.force;
    const t = window.HYBRID_LOADER.timeout || 2000;
  ...
  // 3. Processus hybride intelligent pour chaque script
  async function loadScriptHybrid_OLD(script) {
    // Étape 1: <script src> normal (plus rapide si ça marche)
    if (await tryScriptTag(script.localUrl, script.name)) {
      return true;
    }
