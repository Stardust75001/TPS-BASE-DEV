...
const stickyAnnouncementBarInit = () => {
  ...
  if (!section) return;
  const container = section.closest(".shopify-section");
  ...
  if (bar && typeof bar.clientHeight === "number") {
    navbar.style.top = bar.clientHeight + "px";
  }
};
...
const initializeStoriesAnimated = () => {
  ...
  document.querySelectorAll(".animated-stories-link").forEach((link) => {
    ...
    link.addEventListener("click", (e) => {
      ...
      localStorage.setItem(localStorageKeyTime, String(new Date().getTime()));
    });
  });
};
    ...
    quizItems.querySelectorAll(".btn-quiz-value").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        setTimeout(() => {
          ...
          const titleElement =
            block.nextElementSibling?.querySelector(".title");
          ...
          progressBar.style.width =
            ((Number(block.dataset.question) + 1) /
              ...
              100 +
            "%";
          progressBarText.querySelector("span").textContent = String(
            Number(block.dataset.question) + 1
          );
...
const initializeStickyVideo = () => {
  ...
  const stickySections = document.querySelectorAll(".sticky-video");
  if (!stickySections || stickySections.length === 0) return;
  stickySections.forEach((section) => {
    ...
    if (show) {
      const afterSec = Number(section?.dataset?.showAfter || 0);
      const waitMs =
        Number.isFinite(afterSec) && afterSec > 0 ? afterSec * 1000 : 0;
      setTimeout(() => {
        if (section && section.classList) section.classList.add("show");
      }, waitMs);
    }
    ...
    if (closeBtn)
      closeBtn.addEventListener("click", () => {
        if (section && section.classList) section.classList.remove("show");
        if (video && typeof video.pause === "function") {
          try {
            video.pause();
          } catch (_) {}
        }
        const delayH = Number(section?.dataset?.closingDelay || 0);
        const ms = Number.isFinite(delayH) && delayH > 0 ? delayH * 3600000 : 0;
        try {
